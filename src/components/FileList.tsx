...
const FileList: React.FC<FileListProps> = ({ torrent, torrents, onSelectTorrent, onPlayFile, onRemove, onBack }) => {
  ...
  const fileProgress = (file: TorrentFileInfo) => { if (!file.size) return 0; return Math.min(100, Math.round((file.completed / file.size) * 100)) }
  return <div className="page-content"><button className="back-link" onClick={onBack}>← 返回任务列表</button><div className="page-heading"><div><span className="eyebrow">MEDIA LIBRARY / {torrent.infoHash.slice(0, 8)}</span><h2>{torrent.name}</h2><p>{torrent.files.length} 个文件 · {size(torrent.totalSize)}</p></div><span className={`status-pill ${torrent.status}`}>{statusLabels[torrent.status]} {Math.round(torrent.progress * 100)}%</span></div><div className="thin-progress"><span style={{ width: `${torrent.progress * 100}%` }} /></div><div className="task-actions-row">{torrent.magnetURI && <button className="small-button" onClick={() => void copyMagnet()}>{copied ? '已复制 ✓' : '复制磁力'}</button>}<button className="small-button" onClick={removeKeep}>移除记录</button><button className="small-button danger" onClick={removeWithFiles}>删除任务和文件</button></div><div className="library-tools"><label className="search-box"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索文件名" /></label><span>{files.length} / {torrent.files.length}</span></div>{!torrent.files.length ? <div className="empty-state"><span>◌</span><strong>正在等待文件信息</strong><p>连接到节点后，媒体文件会显示在这里。</p></div> : !files.length ? <div className="empty-state"><strong>没有匹配的文件</strong></div> : <div className="file-table"><div className="file-table-head"><span>文件名</span><span>进度</span><span>类型</span><span>大小</span><span /></div>{files.map((file) => <div className="file-item" key={file.path}><div className="file-name"><span className={`file-icon ${file.type}`}>{icons[file.type]}</span><strong title={file.name}>{file.name}</strong></div><span className="file-progress"><span className="bar"><span style={{ width: `${fileProgress(file)}%` }} /></span><small>{fileProgress(file)}%</small></span><span className="file-type">{labels[file.type]}</span><span className="file-size">{size(file.size)}</span>{file.type === 'video' || file.type === 'audio' ? <button className="play-button" onClick={() => onPlayFile(file)}>播放 <span>↗</span></button> : <span />}</div>)}</div>}</div>
}
